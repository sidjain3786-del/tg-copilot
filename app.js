...
function findStrategy(id){ return DEFAULT_PLAYBOOK.find(p=>p.id===id) || DEFAULT_PLAYBOOK[0]; }
...
async function loadUserData(){
  ...
  STATE.customStrategies = data.customStrategies || [];
}
...
function renderNotesTab(){
  const strategyOptions = DEFAULT_PLAYBOOK.map(p => `<option value="${esc(p.name)}" ${STATE.noteFormStrategy===p.name?'selected':''}>${esc(p.name)}</option>`).join('');
  const customOptions = STATE.customStrategies.map(s => `<option value="${esc(s)}" ${STATE.noteFormStrategy===s?'selected':''}>${esc(s)}</option>`).join('');
  ...
  <div class="card" style="max-width:42rem; margin:0 auto;">
    ...
    <form id="notes-form">
      ...
      <div class="field">
        ...
        <select id="note-strategy-select">
          <optgroup label="Playbook Strategies">${strategyOptions}</optgroup>
          ${STATE.customStrategies.length ? `<optgroup label="My Custom Strategies">${customOptions}</optgroup>` : ''}
          <option value="__custom__">+ Add Custom Strategy...</option>
        </select>
        <div id="note-custom-strategy-row" class="inline-add" style="display:none;">
          <input type="text" id="note-custom-strategy-input" placeholder="Apni strategy ka naam likho...">
          <button type="button" data-action="add-custom-strategy" class="btn-primary" style="padding:.5rem .9rem;">Save</button>
        </div>
      </div>
...
document.addEventListener('click', (e) => {
  ...
  if (action==='set-tab') { STATE.activeTab = btn.dataset.tab; render(); }
    ...
    if (!val) return;
    if (!STATE.customStrategies.includes(val)) STATE.customStrategies.push(val);
